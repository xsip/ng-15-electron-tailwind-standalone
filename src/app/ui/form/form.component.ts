    ...
    <form [formGroup]="fg">
      <div class="grid gap-6 mb-6 md:grid-cols-3">
        <div *ngFor="let key of keys(metaData)">
          ...
          <input type="text" id="{{key}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-slate-500 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="{{metaData[key].placeholder}}">
        </div>
      </div>
    </form>
  `,
